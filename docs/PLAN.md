...
> facility pages that refresh on save — built, `verify-ah`; (5) CSP checked
> in a browser (`verify-ai`), raw IPs removed from the rate limiter (SECURITY
> L5), `/privacy` corrected, docs — built.
...
- One public bucket, `org-media`, every object under `{orgId}/{kind}/…`. The first path segment scopes ownership; the second (`facilities`/`schedules`/`org`) decides whether a write is member- or manager-scoped — all owner/admin. Size and MIME limits are on the bucket, not the client. See `src/components/media/README.md`.
...
**Removed, don't reintroduce without a deliberate decision:** `programs` (collapsed into `schedule_groups`), `scraping_configs`, `scraping_jobs`, `scraping_conflicts` (scraping removed entirely, migration `021`), `seasons`, `session_features`, `brochures`, `brochure_sections`, `brochure_entries` (seasons/events/brochure track removed entirely, migration `036` — see §3a).
...
## 3a. Removed: seasons → event calendar → brochure → control centre

Phases A–D of a five-phase track (specified in full in the now-historical
`docs/prompts/seasons-events-brochure.md`) shipped `seasons`, an `is_event` +
`session_features` event calendar, Storage-backed image upload, and a
`brochures`/`brochure_sections`/`brochure_entries` seasonal brochure — Phase E,
a control centre of milestones and readiness signals, was never started.

**The whole track was torn out 2026-08-16**, the same way scraping was (§1
above). Dropin narrowed twice after this track shipped — first to a
single-organization tool, then again to cut scope aggressively toward
launch — and an eighth-plus staff surface for a feature nobody asked to keep
did not survive the second narrowing. This is a closed chapter, not a paused
one; see `docs/prompts/remove-events-brochure.md` for the removal brief and
`supabase/migrations/036_remove_events_brochure_seasons.sql` for the removal
migration. `docs/RESUME-events.md` is kept as historical record only.

Two general lessons from the build are still worth knowing, since they apply
beyond this one track:

- **`useScheduleAnchor` holds one date, not a week and a month** — deriving the
  week from an anchor is safe; deriving a month back out of a stored week is
  not, since `getWeekStart(getMonthStart(october))` is in September. The hook
  (and its now-unused `month`/`setMonth`) survived the removal for exactly
  this reason.
- **A layout under Cache Components must not `await params`.** It sits above
  the Suspense boundary `loading.tsx` creates, so awaiting request data there
  blocks the segment from prerendering and fails the build outright.

Storage-backed image upload (`org-media`, Storage migration `030`) and the
verification harness pattern at [`scripts/verify/`](../scripts/verify/README.md)
both outlived this track — see §2 and that directory's README.
