...
CREATE OR REPLACE FUNCTION auth.org_role(p_org_id UUID)
...
CREATE POLICY "orgs_public_read_active"
  ...
  USING (status = 'active' OR id = ANY(auth.user_org_ids()) OR auth.is_superadmin());

-- Org owners and admins can update their own org profile
CREATE POLICY "orgs_admin_update"
  ON organizations FOR UPDATE
  USING (
    id = ANY(auth.user_org_ids())
    AND auth.org_role(id) IN ('owner', 'admin')
  );
...
CREATE POLICY "memberships_admin_write"
  ...
  WITH CHECK (
    ...
    AND auth.org_role(org_id) IN ('owner', 'admin')
...
CREATE POLICY "facilities_public_read_published"
  ...
  USING (is_published = TRUE OR org_id = ANY(auth.user_org_ids()) OR auth.is_superadmin());
...
CREATE POLICY "sessions_public_read_active"
  ...
  USING (
    ...
    OR org_id = ANY(auth.user_org_ids())
